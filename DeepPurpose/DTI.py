...
def repurpose(X_repurpose, target, model, drug_names=None, target_name=None,
    ...
    with open(fo, 'w') as fout:
        ...
        df_data = data_process_repurpose_virtual_screening(
            X_repurpose, target, model.drug_encoding, model.target_encoding, 'repurposing')
        ...
        if model.binary:
            table_header = ["Rank", "Drug Name",
                            "Target Name", "Interaction", "Probability"]
        else:
            # regression
            table_header = ["Rank", "Drug Name",
                            "Target Name", "Binding Score"]
        table = PrettyTable(table_header)
        if drug_names is None:
            drug_names = ['Drug ' + str(i)
                          for i in list(range(len(X_repurpose)))]
                ...
                if model.binary:
                    if y_pred[i] > 0.5:
                        string_lst = [drug_names[i], target_name,
                                      "YES", "{0:.2f}".format(y_pred[i])]

                    else:
                        string_lst = [drug_names[i], target_name,
                                      "NO", "{0:.2f}".format(y_pred[i])]
                else:
                    # regression
...
class DBTA:
    ...
    def __init__(self, **config):
        ...
        if drug_encoding == 'Morgan' or drug_encoding == 'Pubchem' or drug_encoding == 'Daylight' or drug_encoding == 'rdkit_2d_normalized' or drug_encoding == 'ESPF':
            ...
            self.model_drug = MLP(
                config['input_dim_drug'], config['hidden_dim_drug'], config['mlp_hidden_dims_drug'])
            ...
            self.model_drug = MPNN(
                config['hidden_dim_drug'], config['mpnn_depth'])
        else:
        ...
        if target_encoding == 'AAC' or target_encoding == 'PseudoAAC' or target_encoding == 'Conjoint_triad' or target_encoding == 'Quasi-seq' or target_encoding == 'ESPF':
            self.model_protein = MLP(
                config['input_dim_protein'], config['hidden_dim_protein'], config['mlp_hidden_dims_target'])
    ...
    def train(self, train, val, test=None, verbose=True):
        ...
        # scheduler = torch.optim.LRScheduler.CyclicLR(opt,base_lr=1e-4,max_lr=1e-2,step_size_up=2000)
        ...
        for epo in range(train_epoch):
            ...
            with torch.set_grad_enabled(False):
                if self.binary:
                    ...
                    lst = ["epoch " + str(epo)] + \
                        list(map(float2str, [auc, auprc, f1]))
                    ...
                    mse, r2, p_val, CI, logits = self.test_(
                        validation_generator, self.model)
                    lst = ["epoch " + str(epo)] + \
                        list(map(float2str, [mse, r2, p_val, CI]))
                    valid_metric_record.append(lst)
                    if mse < max_MSE:
                        model_max = copy.deepcopy(self.model)
                        max_MSE = mse
